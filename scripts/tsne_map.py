...
#    db = DBSession()
    ...
    print(type(which_ids))
    keys_reduced = keys[which_ids]
    ids_reduced = [int(s) for s in keys_reduced]
    X_reduced = X[which_ids,:]
    df_mg = df_mg.set_index('id')
    labels = df_mg['fiber'].loc[ids_reduced].tolist()
    paths = df_mg['imPath'].loc[ids_reduced].tolist()
